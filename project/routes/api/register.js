...
router.post('/', (req, res) => {
    ...
    if (!isEmpty(validation_errors)) {
        return res.status(400).json({ msg: 'Provided fields are incorrect due to the following errors', validation_errors });
    }
            ...
            User.findOne({ email })
                .then(user => {
                    ...
                    if (!isEmpty(account_errors)) {
                        return res.status(400).json({ msg: 'Cannot register this username or email', account_errors });
                    }
